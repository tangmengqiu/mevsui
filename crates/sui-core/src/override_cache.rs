...
impl InputLoaderCache<'_> {
    ...
    fn read_receiving_objects_for_signing(
        ...
        for objref in receiving_objects {
            ...
            let full_id = FullObjectID::new(*object_id, Some(*version));
            let object_key = FullObjectKey::new(full_id, *version);
...
impl ObjectCacheRead for InputLoaderCache<'_> {
    ...
    fn multi_input_objects_available_cache_only(&self, keys: &[InputKey]) -> Vec<bool> {
        self.loader.cache.multi_input_objects_available_cache_only(keys)
    }
    fn notify_read_input_objects<'a>(
        ...
        self.loader.cache.notify_read_input_objects(input_and_receiving_keys, receiving_keys, epoch)
    }
}
